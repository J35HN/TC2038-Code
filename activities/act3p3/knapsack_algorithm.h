...
int KnapsackAlgorith(vector<int> elementValues, vector<int> elementWeights, int maxWeight)
{
    ...
    // Fill our matrix.
    for (int item = 0; item <= elementValues.size(); item++) // current item will be i - 1, due to filling the first row and column with ceros.
    {
        for (int weightValue = 0; weightValue <= maxWeight; weightValue++) // Iterate through each sub-problem max weight.
        {
            ...
            } else if (elementWeights[item - 1] <= weightValue) // If our item fits, see if we use only its value or its value with the last item's value.
            {
